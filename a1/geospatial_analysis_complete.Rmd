---
title: "Geospatial Analysis - Five Mapping Projects"
author: "Your Name"
date: "`r Sys.Date()`"
output: pdf_document
toc: true
...
This project reproduces Figure 2 from Mettetal, E., 2019. *Irrigation dams, water and infant mortality: Evidence from South Africa*. The original study relies on proprietary government GIS data and ArcGIS processing. This replication uses publicly available datasets and open-source R packages, which explains minor visual differences.
...
River data were taken from a publicly available 1:500,000 national river shapefile and combined with a global digital elevation model to calculate river gradients. Rivers were segmented and gradients were aggregated to administrative areas using a length-weighted mean. Dam locations were imported from a public KMZ file and plotted as points. To match the paper as closely as possible, gradients were converted to meters per kilometre and classified using the same break values, resulting in a map that reflects the same geographic patterns despite differences in data sources and methods.
## Code
...
river_pieces <- st_cast(rivers_seg, "MULTILINESTRING") |> st_cast("LINESTRING")
...
labels <- c("0.000000 - 2.229103", "2.263323 - 3.821713", "3.961921 - 6.503167", 
            "6.673425 - 9.879707", "9.942879 - 13.130823", "13.328909 - 19.829017")
districts_grad$grad_bin <- cut(districts_grad$avg_grad_paper, breaks = breaks, 
                               include.lowest = TRUE, right = FALSE, labels = labels)
districts_grad$grad_bin <- factor(districts_grad$grad_bin, levels = labels)

p <- ggplot() +
  geom_sf(data = districts_grad, aes(fill = grad_bin), color = "grey55", linewidth = 0.25) +
  geom_point(data = dams_df, aes(x = x, y = y), color = "black", size = 0.30, alpha = 0.8) +
  ...
  theme(legend.position = c(0.22, 0.83), legend.title = element_text(size = 9), 
...
create_map <- function(data, map_year) {
  ...
  map <- ggplot(year_data) +
    ...
    geom_sf(data = year_data %>% filter(is_western), fill = NA, color = "red", 
            size = 0.8, inherit.aes = FALSE) +
    scale_fill_manual(
      name = "Pop. Share", values = c("1" = "#f1eef6", "2" = "#bdc9e1", "3" = "#74a9cf", 
                                      "4" = "#0570b0", "5" = "#034494"),
      na.value = "white"
    ) +
    labs(title = sprintf("Brazilian Population Distribution by Meso-Region - %d", map_year),
...
ggsave("outputs/meso_region_map_1950.png", map_1950, width = 14, height = 10, dpi = 300, bg = "white")
ggsave("outputs/meso_region_map_1980.png", map_1980, width = 14, height = 10, dpi = 300, bg = "white")
ggsave("outputs/meso_region_map_2010.png", map_2010, width = 14, height = 10, dpi = 300, bg = "white")
```

## Output

![Brazil 1950](outputs/meso_region_map_1950.png){width=95%}

![Brazil 1980](outputs/meso_region_map_1980.png){width=95%}

![Brazil 2010](outputs/meso_region_map_2010.png){width=95%}
